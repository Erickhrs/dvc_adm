...
{
    $sql = "SELECT * FROM users WHERE $coldb = ?";
...
if (isDuplicate($mysqli, 'email', $email)) {
    ...
    ";
} elseif (isDuplicate($mysqli, 'phone', $phone)) {
    echo " <div class=\"container\">
                <div class=\"alert alert-danger\">
                    <h2>ERRO: O telefone informado  já está registrado no banco de dados...</h2>.
                </div>
            </div>";
} elseif (isDuplicate($mysqli, 'CPF', $CPF)) {
    echo " <div class=\"container\">
                <div class=\"alert alert-danger\">
                    <h2>ERRO: O CPF informado já está registrado no banco de dados...</h2>.
                </div>
            </div>";
} else {
    ...
    if (isset($_FILES['picture']) && $_FILES['picture']['error'] === UPLOAD_ERR_OK) {
        ...
        $sql = "INSERT INTO users (name, email, phone, CPF, address, district, city, UF, CEP, picture, birth, since, status, psw) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
