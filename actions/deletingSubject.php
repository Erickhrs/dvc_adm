...
if (isset($_POST) && !empty($_POST) && $_POST['validation'] == 'eu quero excluir o #') {
    ...
    $ID = $_POST['atr_ID'];
    $name = $_POST['atr_name'];
    if (isUtilized($mysqli, 'subject', $ID)){
        echo "<script>alert('ERRO: HÁ QUESTÕES VINCULADAS A ESSE ASSUNTO...')</script>";
        header('Location: ../erro.php');
    }
    else {
        $sql = "DELETE FROM subjects where ID = '$ID'";
        $sql_query = $mysqli->query($sql) or die("Falha na execução do código SQL: " . $mysqli->error);
        newHistoryEvent($_SESSION['id'], "Deletou um assunto (#". $ID . " - " . $name . ")", date('Y-m-d H:i:s'), 'ALTA');
